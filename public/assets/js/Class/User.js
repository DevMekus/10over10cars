...
class UserManager {
  ...
  async security_management() {
    const domForm = document.getElementById("settingsManager");
    const role = Utility.currentPage;
    const userid = Utility.userid;

    if (!domForm) return;

    const url = role == 1 ? `admin/logs` : `user/logs/${userid}`;

    const fetchData = await Utility.fetchData(
      `${Utility.API_ROUTE}/${url}`,
      "GET"
    );
    const logs = fetchData.status == 200 ? fetchData.data : [];

    User.renderLogData(logs);
    User.loginTrends(logs);

    if (role == 1) {
      const fetchUsers = await User.getUser();
      const users = fetchUsers.data && fetchUsers.data;
      const admin = users && users.filter((user) => user.role_id == "1");
      User.renderAdminData(admin);
    }
  }

  become_a_dealer() {
    const domForm = document.getElementById("dealerForm");
    ...
    domForm.addEventListener("submit", async (e) => {
      ...
      if (result.isConfirmed) {
        ...
        //   if (!data.has(`dealer-logo`, logo.files[0])) {
        //     data.append("dealer-logo", logo.files[0]);
        //   }
        // }

        const response = await Utility.fetchData(
          `${Utility.API_ROUTE}/user/profile/dealer`,
          data,
          "POST"
        );
